...
import LoadingSkeleton, { CardSkeleton, TextSkeleton } from './components/LoadingSkeleton';
import { CURATED_TOPICS, CuratedTopic } from './data/curatedTopics';

// Enhanced Curated Topic Card Component with better animations
const CuratedTopicCard: React.FC<{ topic: CuratedTopic; onClick: () => void }> = ({ topic, onClick }) => {
  ...
  return (
    <InteractiveCard
      ...
      hoverEffect="lift"
      glowColor="amber"
      className="text-left bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-lg"
    >
      ...
      <div className="p-5">
        <h4 className="font-bold text-white text-base mb-2 group-hover:text-amber-400 transition-colors duration-300">{topic.title}</h4>
        <p className="text-zinc-500 text-sm leading-relaxed group-hover:text-zinc-400 transition-colors duration-300">{topic.tagline}</p>
        <div className="w-0 h-0.5 bg-gradient-to-r from-amber-400 to-orange-500 group-hover:w-full transition-all duration-500 mt-3" />
      </div>
    </InteractiveCard>
  );
};
    ...
    return (
      <div className="h-screen bg-zinc-950 flex">
            ...
            <div className="flex items-center gap-3">
              ...
              {showHistorySidebar && (
                <div className="overflow-hidden">
                  <h1 className="font-bold text-white text-lg">HV LearnOS</h1>
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Hero Section with Enhanced Backgrounds - Fixed Height */}
          <div className="relative min-h-[80vh] flex flex-col items-center justify-center p-6 overflow-hidden">
            ...
            <div className="z-20 max-w-xl w-full text-center space-y-8 animate-fade-in-up">
            {/* Mobile Logo */}
            <div className="md:hidden flex justify-center mb-4">
              <div className="flex items-center gap-3">
                ...
                <h1 className="font-bold text-white text-2xl">HV LearnOS</h1>
              </div>
            </div>
            ...
            {isLoading && (
              <div className="text-center space-y-4 animate-fade-in-up">
                <div className="w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center mx-auto animate-pulse-glow">
                  <Icons.Sparkles className="text-black animate-spin" />
                </div>
                <div className="space-y-2">
                  <p className="text-amber-400 font-medium animate-typing">{loadingText}</p>
          ...
          <div className="bg-zinc-950 border-t border-zinc-900 px-6 md:px-12 py-16">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {CURATED_TOPICS.map((curatedTopic, index) => (
                  <div
                    key={curatedTopic.id}
    ...
    return (
      <div className="flex h-screen bg-zinc-950 overflow-hidden text-white">
        ...
        <div className="flex-1 flex flex-col h-full relative bg-zinc-950">
          ...
          {/* Enhanced Bottom Nav with better styling */}
          <div className="h-20 bg-gradient-to-r from-zinc-950/95 to-zinc-900/95 backdrop-blur-sm border-t border-zinc-800 flex items-center justify-between px-6 absolute bottom-0 w-full z-20 shadow-2xl">
            ...
            <div className="flex items-center gap-2">
              {course.modules.map((module, modIdx) => (
                module.slides.map((_, slideIdx) => {
                  const isActive = modIdx === activeModuleIndex && slideIdx === activeSlideIndex;
                  const isPast = modIdx < activeModuleIndex || (modIdx === activeModuleIndex && slideIdx < activeSlideIndex);
                  return (
                    <button
                      key={`${modIdx}-${slideIdx}`}
                      onClick={() => jumpToSlide(modIdx, slideIdx)}
                      className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        isActive 
                          ? 'bg-amber-400 scale-150 shadow-amber-400/50 shadow-lg' 
                          : isPast 
            ...
            <FloatingActionButton
              ...
              label={isLast ? "Course Complete!" : "Next Slide"}
              color="primary"
              size="md"
              pulse={!isLast}
            />
